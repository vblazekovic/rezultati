...
kat_vals = sorted([k for k in df[col_kat].dropna().unique().tolist()]) if col_kat in df.columns else []
kat = st.sidebar.multiselect("Kategorija", kat_vals, default=[])

klub_vals = sorted([k for k in df[col_klub].dropna().unique().tolist()]) if col_klub in df.columns else []
